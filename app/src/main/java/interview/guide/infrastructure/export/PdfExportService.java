...
public class PdfExportService {
     ...
     * 导出语音面试评估报告为 PDF
     */
    public byte[] exportVoiceInterviewReport(VoiceEvaluationDetailDTO evaluation) {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        PdfWriter writer = new PdfWriter(baos);
        PdfDocument pdfDoc = new PdfDocument(writer);
        Document document = new Document(pdfDoc);
        ...
        Paragraph title = new Paragraph("语音面试评估报告")
            .setFontSize(24)
            .setBold()
            .setTextAlignment(TextAlignment.CENTER)
            .setFontColor(HEADER_COLOR);
        ...
        document.add(new Paragraph("\n"));
        document.add(createSectionTitle("面试信息"));
        document.add(new Paragraph("语音会话 ID: " + evaluation.getSessionId()));
        document.add(new Paragraph("题目数量: " + evaluation.getTotalQuestions()));

        document.add(new Paragraph("\n"));
        document.add(createSectionTitle("综合评分"));
        Paragraph scoreP = new Paragraph("总分: " + evaluation.getOverallScore() + " / 100")
            .setFontSize(18)
            .setBold()
            .setFontColor(getScoreColor(evaluation.getOverallScore()));
        document.add(scoreP);

        if (evaluation.getOverallFeedback() != null) {
            document.add(new Paragraph("\n"));
            document.add(createSectionTitle("总体评价"));
            document.add(new Paragraph(sanitizeText(evaluation.getOverallFeedback())));
        }

        if (evaluation.getStrengths() != null && !evaluation.getStrengths().isEmpty()) {
            document.add(new Paragraph("\n"));
            document.add(createSectionTitle("表现优势"));
        ...
        if (evaluation.getImprovements() != null && !evaluation.getImprovements().isEmpty()) {
            document.add(new Paragraph("\n"));
            document.add(createSectionTitle("改进建议"));
            for (String improvement : evaluation.getImprovements()) {
                document.add(new Paragraph("- " + sanitizeText(improvement)));
            }
        }

        List<VoiceEvaluationDetailDTO.AnswerDetail> answers = evaluation.getAnswers();
        if (answers != null && !answers.isEmpty()) {
            document.add(new Paragraph("\n"));
            document.add(createSectionTitle("问答详情"));

            for (VoiceEvaluationDetailDTO.AnswerDetail answer : answers) {
                document.add(new Paragraph("\n"));
                document.add(new Paragraph("问题 " + (answer.getQuestionIndex() + 1)
                    + " [" + (answer.getCategory() != null ? answer.getCategory() : "综合") + "]")
                    .setBold()
                    .setFontSize(12));
                document.add(new Paragraph("Q: " + sanitizeText(answer.getQuestion())));
                document.add(new Paragraph("A: " + sanitizeText(answer.getUserAnswer() != null
                    ? answer.getUserAnswer() : "未回答")));
                document.add(new Paragraph("得分: " + answer.getScore() + "/100")
                    .setFontColor(getScoreColor(answer.getScore())));
                if (answer.getFeedback() != null) {
                    document.add(new Paragraph("评价: " + sanitizeText(answer.getFeedback()))
                ...
                if (answer.getKeyPoints() != null && !answer.getKeyPoints().isEmpty()) {
                    document.add(new Paragraph("关键点:"));
                    for (String keyPoint : answer.getKeyPoints()) {
                        document.add(new Paragraph("- " + sanitizeText(keyPoint)));
                    }
                }
            }
        }
