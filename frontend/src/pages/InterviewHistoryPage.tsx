...
export default function InterviewHistoryPage({ onBack: _onBack, onViewInterview, onRestartInterview, onContinueInterview }: InterviewHistoryPageProps) {
  ...
  const loadAll = useCallback(async (isPolling = false) => {
    ...
    try {
      ...
      const scored = evaluated.filter(i => i.overallScore !== null);
      const totalScore = scored.reduce((sum, i) => sum + (i.overallScore || 0), 0);
      const newStats = {
  ...
  const handleExport = async (item: UnifiedInterviewItem, e: React.MouseEvent) => {
    e.stopPropagation();
    const exportKey = item.id;
    setExporting(exportKey);
    try {
      const isVoice = item.type === 'voice';
      if (isVoice && !item.voiceSessionId) {
        throw new Error('Missing voice session id');
      }

      const blob = isVoice
        ? await voiceInterviewApi.exportEvaluationPdf(item.voiceSessionId!)
        : await historyApi.exportInterviewPdf(item.sessionId);
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      const filenamePrefix = isVoice ? '语音面试报告' : '面试报告';
      const filenameSuffix = isVoice ? String(item.voiceSessionId) : item.sessionId.slice(-8);
      a.href = url;
      a.download = `${filenamePrefix}_${filenameSuffix}.pdf`;
