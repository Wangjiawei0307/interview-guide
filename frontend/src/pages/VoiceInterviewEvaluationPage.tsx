...
export default function VoiceInterviewEvaluationPage() {
  ...
  const handleExportPdf = async () => {
    ...
    setExporting(true);
    try {
      const blob = await voiceInterviewApi.exportEvaluationPdf(parseInt(sessionId));
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `语音面试报告_${sessionId}.pdf`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
    } catch (err) {
      console.error('Failed to export voice evaluation PDF:', err);
      alert('导出失败，请重试');
    } finally {
      setExporting(false);
    }
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-900 dark:text-white">语音面试评估报告</h1>
          ...
          <button
            ...
            disabled={exporting}
            className="inline-flex items-center justify-center gap-2 px-4 py-2 bg-primary-500 text-white rounded-lg hover:bg-primary-600 disabled:opacity-60 disabled:cursor-not-allowed transition-colors"
          >
            {exporting ? (
